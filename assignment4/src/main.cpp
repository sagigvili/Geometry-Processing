...
Eigen::VectorXi total_vertices, free_vertices;
...
Eigen::MatrixXd Vb;
Eigen::MatrixXd Vb_tag;
Eigen::MatrixXd Vs_tag;
Eigen::MatrixXd V_origin;
...
bool solve(Viewer& viewer)
{
    ...
    Eigen::MatrixXd origin_handle_positions = igl::slice(V_origin, handle_vertices, 1),
                    Vf = solver.solve(-Afc * origin_handle_positions);

    igl::slice_into(Vf, free_vertices, 1, B);
    Vb = B;
    ...
    for (int i = 0; i < V.rows(); ++i) {
        double maxDist = 0;
        Eigen::RowVector3d ni = N.row(i), xi(3), yi(3);
        ni.normalize();
        for (int j = 0; j < VV[i].size(); ++j) {
            Eigen::RowVector3d vi = B.row(VV[i][j]), proj(3);
        ...
        xi.normalize();yi = xi.cross(ni); yi.normalize();
        Eigen::RowVector3d di = D.row(i);
    ...
    D.resize(V.rows(), 3);
    for (int i = 0; i < V.rows(); ++i) {
        Eigen::RowVector3d vi = neighbors.row(i), ni = N.row(i), xi(3), yi(3);
        ni.normalize();
        xi = vi - vi.dot(ni) * ni;
        xi.normalize();
        yi = xi.cross(ni); yi.normalize();
        D.row(i) = localD(i, 0) * xi + localD(i, 1) * yi + localD(i, 2) * ni;
    }

    Eigen::MatrixXd Vout = B + D;
    Vb_tag = B;
    Vs_tag = Vout;
    V = Vout;
    return true;
...
bool callback_mouse_move(Viewer& viewer, int mouse_x, int mouse_y)
{
    ...
    if ((mouse_mode == TRANSLATE) || (mouse_mode == ROTATE))
    {
        ...
        cout << "Exec time is " << deltaTime / 1000 << endl;
        return true;
...
bool callback_key_down(Viewer& viewer, unsigned char key, int modifiers)
{
    ...
    if (key == 'A')
    {
     ...
     if (key == '3') {
       // S' detailed mesh after trasnformations
       V = Vs_tag;
     }
    return handled;
}
...
void applySelection()
{
    ...
    Eigen::SparseMatrix<double> Lw, M, Minv, A;
    ...
    free_vertices.resize(V.rows() - handle_vertices.rows());
    int count = 0;
    for (int i = 0; i < V.rows(); ++i) {
        if (handle_id(i) == -1) {
    ...
    igl::cotmatrix(V, F, Lw); Lw = -Lw;
    igl::massmatrix(V, F, igl::MASSMATRIX_TYPE_BARYCENTRIC, M);
    Minv = M.cwiseInverse();  A = Lw * Minv * Lw;
